...
class InterestHobbiesForm extends Form {
	...
	componentDidMount() {
		...
		let data = { ...this.state };
		if (currentResume.interest) {
			data.mainContent = this.generateDoc(currentResume.interest);
		}
	...
	schema = {
		...
		interest: {
			id: Joi.number(),
			name: Joi.string().required().label("Interest & Hobbies"),
		},
	};
	...
	handleAddMore = (type) => {
		...
		if (Object.keys(errors).length > 0) {
			...
			if (mainContent.interest.length >= 5) {
				return this.setState({
					customErr: "You can add maximum of 5 Interest & Hobbies.",
				});
			} else {
				this.setState({ customErr: "" });
			}
			const interest = {
				id: this.state.mainContent.interest.length + 1,
				name: "",
			};
			mainContent.interest.push(interest);
	...
	interest() {
		...
		return (
			<form onSubmit={this.handleSubmit("interest")}>
				<div className="row mb-2">
					...
					<div className="container">
						<div className="row">
							{mainContent.interest.map((interest, index) => (
								<div key={index} className="col-md-6 my-2">
									<InputField
										handleChange={this.handleChange("interest")}
										error={errors["name" + interest.id]}
										value={interest.name}
										type="text"
										id={`name${interest.id}`}
										autoFocus
						...
						<div className="row px-3">
							...
							<div className="col-1 px-0">
								<Tooltip placement="bottom" title="Add More" arrow>
									<IconButton
										onClick={() => this.handleAddMore("interest")}
										aria-label="add more"
									>
...
export default InterestHobbiesForm;
